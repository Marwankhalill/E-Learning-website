...
export class EnrollmentsService {
  ...
  async findEnrollments(owner: MongoIdDto) {
    ...
    // Populate courses and calculate progress for each
    const enrolledCourses = await Promise.all(
      (user.enrolledCourses || []).map(async (courseId: Types.ObjectId) => {
        const course = await this.courseModel.findById(courseId).lean();
        if (!course) return null;

        const courseIdStr = courseId.toString();
        // lessonProgress is stored as a Map in Mongoose, but when accessed it's a Map object
        const lessonProgressMap = user.lessonProgress || new Map();
        const viewedLessons = lessonProgressMap.get(courseIdStr) || [];
